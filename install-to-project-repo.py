...
def parse_by_eclipse_standard(path):
  ...
  if match != None:
    ...
    javadoc = name[-1] == "javadoc"
    (group, name) = (".".join(name[:-2]), name[-2]) if source or javadoc else (".".join(name[:-1]), name[-1])
...
def install(path, parsing):
  print "Choose a correct packaging for `%s`:" % path
  labels = ['jar', 'so', 'dll']
  (packaging) = input_choice(labels, labels)
  ...
  os.system(
    ...
    (" -Dclassifier=javadoc" if parsing["javadoc"] else "")
  )
...
def group_to_name_alternatives(group):
  return [
    ...
    if (v.lower()) not in ["source", "snapshot", "javadoc"]
  ]
...
def version_parsing(version):
  ...
  (version, javadoc) = f(version, "-", "javadoc")
  (version, snapshot) = f(version, "-", "SNAPSHOT")
  if not snapshot:
    (version, snapshot) = f(version, ".", "SNAPSHOT")

  return version, snapshot, source, javadoc

def name_parsing(name):
  if name.endswith(".source"):
    return name[:-len(".source")], True, False
  elif name.endswith(".javadoc"):
    return name[:-len(".javadoc")], False, True
  else:
...
def parse_interactively(path):
  ...
  version, snapshot, source, javadoc = version_parsing(version)
  name, source1, javadoc1 = name_parsing(name)
